...
import {getAllInformacion} from "../../api/DatosAPI.api.js";

// eslint-disable-next-line react/prop-types
export function DatosExtraPage({setUser}) {
  ...
  useEffect(() => {
    async function loadTasks() {
      return await getAllInformacion();
    }
        ...
        <div className="w-4/6 mx-auto">
          ...
          <div className="flex justify-end">
            ...
            <button className="bg-dark-purple text-white p-2 rounded-lg w-1/6 ml-2"
                    onClick={
                      async () => {
                        navigate("/estimacion/datos-create")
                      }
                    }
            >
              Agregar
          ...
          <div className="flex">
            <div className="flex w-full">
              ...
              <input
                  ...
                  type="text"
                  placeholder="Buscar por Nombre de Datos"
          ...
          <table className="w-full mt-4">
            ...
            <tr>
              ...
              <th className='p-3 text-l font-semibold tracking-wide text-left'>Nombre Valor</th>
              <th className='p-3 text-l font-semibold tracking-wide text-left'>Exclusivo de Producto</th>
            ...
            {currentItems.length > 0 ? (
                    currentItems.map((dato, index) => (
                        <tr key={index} className="bg-zinc-100 p-3  rounded-lg odd:bg-white even:bg-slate-50">
                          <td className='p-3 text-gray-700'>{dato.id}</td>
                          <td className='p-3 text-gray-700'>{dato.cabeceraInformacionExtra}</td>
                          <td className='p-3 text-gray-700'>{dato.exclusivaProducto ? "Si" : "No"}</td>
                          <td className='p-3 text-gray-700'>{dato.activo ? "Activo" : "Inactivo"}</td>
                          <td className='p-3'>
                            <button className="bg-dark-purple p-2 rounded-lg text-white text-sm"
                                    onClick={
                                      () => {
                                        navigate(`/estimacion/datos/${dato.id}`)
                                      }
                                    }
                            >
                              Editar
